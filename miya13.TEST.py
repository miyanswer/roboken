...
def main():
    ...
    try:
        while True:
            ...
            # 赤色と青色のマスクを作成
            #lower_red = np.array([1, 150, 150])  # 色相、彩度、明度
            #upper_red = np.array([15, 255, 255]) #2,195,220
            ...
            #upper_blue = np.array([115, 255, 135]) #107,225,100-105
            ...
            cv2.line(output_img, (center_x + 160, 0), (center_x + 160, height), (255, 255, 255), 2)
            #中央から70ピクセル下の横線
            cv2.line(output_img, (0, center_y + 70), (width, center_y + 70), (255, 255, 255), 2)
            ...
            # 赤い領域の輪郭を黄色で囲む（面積が200平方ピクセル以上の場合のみ）
            for contour in red_contours:
                if cv2.contourArea(contour) > 150:
                    ...
                    # 同じシリアルが送信されないようにする
                    # 中心から左右160ピクセル以内のとき'w'を送信
                    if center_x - 160 <= red_center_x <= center_x + 160:
                        current_command = 'w'
                    # 画面中心から160ピクセル以上右の場合に'r'を送信  
                    elif red_center_x > center_x + 160:
                        current_command = 'r'
                    # 画面中心から160ピクセル以上左の場合に'l'を送信
                    elif red_center_x < center_x -160:
                        current_command = 'l'
